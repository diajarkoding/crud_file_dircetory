...
import 'package:convert_widget/fix-vocher-acces.dart';
...
// Create directory inside where file will be saved
  ...
  Future saveImage3(Uint8List bytes) async {
    ...
    final file = File('${appStorage!.path}/voucher-smn.png');
  ...
  Future saveImage4({
    ...
    const rootAppFolder = 'Voucher SMN';
    ...
    try {
      ...
      if (storage == PermissionStatus.granted) {
        ...
        if (await directory.exists()) {
          ...
          debugPrint('Berhasil menyimpan voucher di : ${file.path}');
        }
      }
    } catch (e) {
      debugPrint('Error karena : $e');
    }
  }

  Future<bool> saveImage5({
    required Uint8List bytes,
    required String fileName,
  }) async {
    const rootAppFolder = 'Voucher SMN';
    ...
    try {
      ...
      var data = await file.writeAsBytes(bytes);

      if (data.existsSync()) {
        debugPrint('Berhasil menyimpan voucher di : ${file.path}');
        return true;
      } else {
        debugPrint('Gagal menyimpan voucher di : ${file.path}');
        return false;
      }
    } catch (e) {
      debugPrint('Error karena : $e');
  ...
  Future<void> chekSDK(
    ...
    debugPrint('Android $release (SDK $sdkInt)');
